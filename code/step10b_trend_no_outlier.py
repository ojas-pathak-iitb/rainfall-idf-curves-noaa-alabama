"""
IDF Curve Project
Step 10b: Temporal Trend Analysis — Outlier Removed (Clean Version)

What this does:
    - Same as step10 but with the confirmed 1971 outlier removed
    - The year 1971 was confirmed as a statistically significant outlier
      by the Grubbs test in step11 (G = 5.87, G_crit = 3.07)
    - Removing this year gives the "clean" temporal trend
    - Splits into:
        Period 1: 1972-1992 (21 years, 1971 excluded)
        Period 2: 1993-2013 (21 years)
    - Compare this plot with step10 to see how much the 1971 outlier
      was distorting the apparent trend for short durations

Input  : output/ams.csv
Output : output/trend_analysis_clean.png
         output/trend_comparison_clean.csv
"""
...
OUTPUT_PNG = os.path.join(OUTPUT_DIR, "trend_analysis_clean.png")
OUTPUT_CSV = os.path.join(OUTPUT_DIR, "trend_comparison_clean.csv")

# Remove 1971 — confirmed outlier year across 4 durations
OUTLIER_YEAR   = 1971
SPLIT_YEAR     = 1992
...
ams_clean = ams[ams.index != OUTLIER_YEAR]

period1 = ams_clean[ams_clean.index <= SPLIT_YEAR]
period2 = ams_clean[ams_clean.index >  SPLIT_YEAR]

print(f"Period 1: {period1.index.min()}--{period1.index.max()} "
      f"({len(period1)} years, {OUTLIER_YEAR} removed)")
...
for dur in durations:
    for label, period in [("1972-1992", period1), ("1993-2013", period2)]:
...
print("\nPercentage change Period 1 to Period 2 (outlier removed):")
print("-" * 65)
print(f"{'Duration':<10} {'T=2':>8} {'T=5':>8} {'T=10':>8} "
      f"{'T=25':>8} {'T=50':>8} {'T=100':>8}")
print("-" * 65)
for dur in durations:
    p1  = trend_df[(trend_df["period"] == "1972-1992") &
...
for ax, dur, dlabel in zip(axes, DUR_ORDER, DUR_LABELS):
    ...
    ax.plot(T, i1, "o-", color=COLOR1, linewidth=2.0,
            markersize=6, label="1972–1992 (outlier removed)")
...
fig.suptitle(
    f"Temporal Trend Analysis — Outlier Removed ({OUTLIER_YEAR} event excluded)\n"
    "Alabama Station 01014000/01014007 | "
    "Period 1: 1972–1992 vs Period 2: 1993–2013",
    fontsize=12, y=1.01)
